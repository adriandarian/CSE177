...
Comparison::Comparison(const Comparison &copy_me) : operand1(copy_me.operand1), whichAtt1(copy_me.whichAtt1),
...
ostream &operator<<(ostream &_os, OrderMaker &_o)
{
	...
	{
		_os << _o.whichAtts[i] << ":";
		if (_o.whichTypes[i] == Integer)
			_os << "Integer";
		else if (_o.whichTypes[i] == Float)
			_os << "Float";
		else
			_os << "String";
