...
template <class Type>
void Keyify<Type>::Swap(Keyify &withMe)
{
	SWAP(data, withMe.data);
}

template <class Type>
void Keyify<Type>::CopyFrom(Keyify &withMe)
{
	data = withMe.data;
}

template <class Type>
int Keyify<Type>::IsEqual(Keyify &checkMe)
{
	return (data == checkMe.data);
}

template <class Type>
int Keyify<Type>::LessThan(Keyify &checkMe)
{
	return (data < checkMe.data);
}
